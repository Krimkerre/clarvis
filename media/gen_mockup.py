...
CSS = '''
  /* ---- planning mockup ---- */
  .chip-plan { margin-left: auto; font-size: 10.5px; color: #ffb648;
    background: rgba(255,182,72,.1); border: 1px solid rgba(255,182,72,.35);
    border-radius: 10px; padding: 1px 8px; letter-spacing: .04em; }
  .thread { flex: 1 1 auto; min-height: 0; overflow: hidden;
    padding: 10px 14px 0; display: flex; flex-direction: column; gap: 9px; }
  .msg { max-width: 92%; padding: 8px 11px; border-radius: 9px; font-size: 12px; line-height: 1.5; }
  .msg.them { align-self: flex-start; background: #12151b; border: 1px solid #232838;
    color: #b6c0cd; border-bottom-left-radius: 3px; }
  .msg.me { align-self: flex-end; background: #2a3444; color: #dbe4ee; border-bottom-right-radius: 3px; }
  .msg ol { margin: 6px 0 0; padding-left: 17px; }
  .msg li { margin-bottom: 5px; }
  .msg .em { color: #cdd6e3; font-style: italic; }
  .finding { margin: 3px 14px 0; background: #1b2430; border: 1px solid #2f4a63;
    border-left: 3px solid #4aa3df; border-radius: 7px; padding: 10px 12px; }
  .finding .hd { font-size: 10px; color: #4aa3df; letter-spacing: .07em; font-weight: 600; margin-bottom: 5px; }
  .finding .body { font-size: 11.5px; color: #c3ced9; line-height: 1.5; }
  .finding .fix { font-size: 11.5px; color: #9ec9a4; margin-top: 6px; line-height: 1.45; }
  .finding .btns { display: flex; gap: 6px; margin-top: 9px; }
...
def page_planning(anim):
    d = ((lambda t: ' style="animation-name:seq%s"' % str(t).replace('.', '_'))
         if anim else (lambda t: ''))
    seq = " seq" if anim else ""
    face = f'''<svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">{DEFS}
        <g class="{'bob' if anim else ''}">{avatar("talking")}</g></svg>'''

    return f'''<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Clarvis — planning mockup</title>
<style>{CSS}{ANIM_CSS if anim else ""}</style></head>
<body>
<div class="titlebar">
  <div class="dots"><div class="dot" style="background:#ff5f57"></div><div class="dot" style="background:#febc2e"></div><div class="dot" style="background:#28c840"></div></div>
  plan.md — wayfinder
...
open(os.path.join(OUT, "mockup-planning.html"), "w").write(page_planning(anim=True))
open(os.path.join(OUT, "_planning_static.html"), "w").write(page_planning(anim=False))
print("wrote mockup-planning.html + _planning_static.html")
